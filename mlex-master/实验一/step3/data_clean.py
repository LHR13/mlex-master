...
data['cast_total_facebook_likes'] = data['cast_total_facebook_likes'].astype(
    float64)  # 将cast_total_facebook_likes改为float64格式
print(data.dtypes)
...
def normal_distribution_exception_handling(data):
    ...
    for i in range(len(data)):
        if (upper < data[i]) | (lower > data[i]):
            data[i] = mean_data
    return pd.DataFrame(data)


normal_distribution_exception_handling(data['actor_2_facebook_likes'])

data2 = data.to_csv("C:\\Users\\李昊儒\\Desktop\\预处理  标准化，可视化，PCA实验数据\\3out.csv")
